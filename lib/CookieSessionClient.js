...
class CookieSessionClient {
    ...
    async fetchFile (url, rawOptions = {}) {
        ...
        fileDownloadOptions.dir = this.dirFiles

        for (const [k, v] of Object.entries (rawOptions)) {

            if (k === 'subDir') {

                for (const p of this.normalizeSubDir (v))
                    
                    fileDownloadOptions.dir = Path.join (fileDownloadOptions.dir, p)

            }
            else {

                (k in fileDownloadOptions ? fileDownloadOptions : options) [k] = v

            }

        }

        fs.mkdirSync (fileDownloadOptions.dir, {recursive: true})
