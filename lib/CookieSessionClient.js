...
class CookieSessionClient {
    ...
    async invoke (clazz, url, options = {}) {

        options.loggingId = this.newID ()
        
        for (let i = 0; i < 2; i ++) {

            const response = await (new clazz (this, url, options)).perform ()

            if (!this.isUnauthorized (response)) return response
    ...
    async fetch (url, options) {
        ...
        return this.invoke (FetchRequst, url, options)
